...
using Microsoft.SemanticKernel.Memory.Redis; // Using Redis as our vector store
...
Console.ReadLine();
Console.WriteLine("***************************************");
Console.WriteLine($"llama3.2's answer with RAG (using vector store):");
Console.WriteLine("");

// Initialize the embedding generator and create a vector store using Redis
...
var redisMemoryStore = new RedisMemoryStore(redisConnectionString);
var memory = new SemanticTextMemory(redisMemoryStore, embeddingGenerator);
...
foreach (var rate in exchangeRates)
{
    ...
    // Save information into the vector store
...
Console.WriteLine($"{counter} rates added to the vector store memory... please wait");

// Import the vector store memory as a plugin to enable retrieval in prompts
